...
function extractSymbols(code) {
  ...
  const symbols = [];
  let id = 0;
  const excluded = new Set(["if","for","while","switch","catch","return","throw","new","else","import","export","from","default","typeof","instanceof","in","of","try","finally","class","function","const","let","var","async","await","yield","break","continue","case","delete","void","this","super","true","false","null","undefined","NaN","Infinity","type","interface","enum","keyof","extends","implements","as","is","readonly","abstract","static","private","public","protected","declare","namespace","module","global"]);
  ...
  const patterns = [
    ...
    { re: /(?:export\s+)?namespace\s+(\w+)/g, kind: "namespace" },
    { re: /(?:export\s+)?(?:async\s+)?(?:static\s+)?(\w+)\s*\([^)]*\)\s*:\s*\w+/g, kind: "method" },
  ];
  ...
  for (const { re, kind } of patterns) {
    ...
    while ((m = re.exec(cleaned)) !== null) {
      const name = m[1];
      if (excluded.has(name) || name.length < 2) continue;
      const key = `${name}:${kind}`;
      ...
      symbols.push({
        id: `s${id++}`, kind, name, sig: sigLine || `${kind} ${name}`,
  ...
  let im; while ((im = importRe.exec(code)) !== null) imports.push(im[0].trim().slice(0, 100));
...
function resolveSymbol(code, symbolId) {
  ...
  const result = []; let depth = 0, started = false;
...
function compress(code, aliasMap = { to: {}, from: {} }) {
  let out = stripComments(code).replace(/[ \t]+/g, " ").replace(/^ +/gm, "").replace(/\n{2,}/g, "\n").trim();
