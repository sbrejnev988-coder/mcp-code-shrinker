...
import { buildContextPacket, canonicalRepoPath } from "./compiler/packet-builder.js";
...
const toolDefs = [
  ...
  { name: "file.contracts", title: "File Contracts (L1)", description: "Layer 1: contracts for all symbols. FIXED: includes full body ranges.", inputSchema: { type: "object", properties: { repository_id: { type: "string", minLength: 1 }, filePath: { type: "string" } }, required: ["repository_id","filePath"] } },
...
async function handleContextCreate(args) {
  ...
  const legacyRepositoryId = String(args.task?.repositoryId || "").trim();
  ...
  if (!packet.qualitySatisfied && args.allowRecovery !== false) {
    ...
    if (recoveryPlan.length > 0) {
      try {
        const recovered = await buildContextPacket({ ...args, repositoryId: repoId, 
          targetFile: tf,
...
async function handleContextExpand(args) {
  ...
  for (const req of (args.requests || [])) {
    ...
    } else if (packet._targetFile) {
      fp = await resolveInsideRoot(packet._targetFile);
      if (!isInside(slot.root, fp)) {
        throw new Error(`CROSS_REPOSITORY_EXPAND: ${fp} outside ${slot.root}`);
      }
    } else continue;
    try {
      const parsed = parseFile(fp);
      ...
      const c = extractContract(sym, parsed.code, parsed.language);
      const sid = symId(slot.root, fp, parsed, sym);
      ...
      if (!packet.packet.contracts.find(x => x.id === sid)) {
        packet.packet.contracts.push({ handle: h, id: sid, revision: srev, file: fp, file_path: canonicalRepoPath(slot.root, fp), kind: sym.kind, signature: c.signature, effects: c.effects, throws: c.throws, properties: c.properties, confidence: c.confidence, range: [sym.startLine, sym.endLine] });
      ...
      if (req.view === "source" && c.body && !packet.packet.sources.find(x => x.id === sid)) {
        packet.packet.sources.push({ handle: h, id: sid, expectedRevision: srev, file: fp, file_path: canonicalRepoPath(slot.root, fp), language: parsed.language, source: c.body });
  ...
  for (const src of sources) {
    ...
    packet.coverage_manifest.covered.push({
      kind: "exact_source", file_path: src.file_path || (src.file ? canonicalRepoPath(slot.root, src.file) : ""),
  ...
  for (const contract of contracts) {
    ...
    packet.coverage_manifest.covered.push({
      kind: "contract", file_path: contract.file_path || (contract.file ? canonicalRepoPath(slot.root, contract.file) : ""),
